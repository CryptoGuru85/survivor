...
function updateUserByTournament(email, league, tournamentId) {
    return  User.findOne({email: email.toLowerCase(), 'league.name':league}).then((user) => {
        tournamentIndex = user.league.findIndex(tournament => tournament.name == league);
        user.league[tournamentIndex].tournament.push({ tournament_id: tournamentId, flg_out: false });
        user.save();
    })
}

function getUserByLeagueName(leagueName) {
    return User.find({'league.name': {$lte: leagueName}, 'league.status':{$lte:'ACTIVE'}}).lean();
}
...
const loadData = async() =>{
    ...
    return User.collection.insertMany(data);
}
